...
Among adults 18 years or older, `r full_dat %>% filter(confagey > 17, approved == "approved") %>% nrow()` participants had a blood test that was approved.

Meanwhile, `r full_dat %>% filter(confagey > 17, approved == "failure") %>% nrow()` adults had a blood test that failed.

Note that a participant can have a failed and approved blood test if the test was run again successfully after a failure.
